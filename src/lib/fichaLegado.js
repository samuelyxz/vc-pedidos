...
// Modelo de ficha cadastral anterior (.xlsx), usado até setembro/2026. Fica no
// app como alternativa caso algum cliente ainda precise sair no formato antigo;
// o modelo em vigor é o de lib/ficha.js.
...
// Preenche o .xlsx original no lugar (edição cirúrgica do XML — preserva
// layout, caixas de seleção, formatação, logo e fórmulas).
/** @param {Record<string, string>} form @returns {Promise<Uint8Array>} */
export async function montarFichaLegada(form) {
...
/** @param {Record<string, string>} form @param {string} nomeArquivo */
export async function exportarFichaLegada(form, nomeArquivo) {
  downloadBlob(await montarFichaLegada(form), nomeArquivo, XLSX_MIME);
}


/** Baixa o modelo antigo em branco, intocado. */
export function baixarFichaLegadaEmBranco() {
  downloadBlob(
    b64ToUint8(FICHA_CADASTRO_B64),
    'Ficha_Cadastro_EM_BRANCO_modelo_antigo.xlsx',
    XLSX_MIME
  );
}
